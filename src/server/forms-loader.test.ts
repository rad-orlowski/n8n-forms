...
describe("loadFormsFromDir", () => {
  it("returns empty result for a missing directory", async () => {
    const result = await loadFormsFromDir("/tmp/__does_not_exist_ever__");
  ...
  // -------------------------------------------------------------------------
  it("parses a valid JSON5 form file", async () => {
    const payload = validForm("contact");
    write("contact.form.json5", `// json5 comment\n${JSON.stringify(payload)}`);
    const { forms, rejected } = await loadFormsFromDir(dir);
  ...
  // -------------------------------------------------------------------------
  it("parses a valid YAML form file", async () => {
    write(
      "signup.form.yaml",
      `slug: signup\ntitle: Sign Up\npages:\n  - fields:\n      - type: text\n        name: email\n`,
    );
  ...
  // -------------------------------------------------------------------------
  it("rejects a file that fails FormSchema validation", async () => {
    ...
    expect(rejected[0].reason).toMatch(/title/i);
  });

  // -------------------------------------------------------------------------
  // 5. Rejects a file with malformed JSON5/YAML (parse error)
  // -------------------------------------------------------------------------
  it("rejects a file with a parse error", async () => {
  ...
  // -------------------------------------------------------------------------
  it("rejects all files sharing a duplicate slug", async () => {
    ...
    expect(reasons.every((r) => /slug/i.test(r))).toBe(true);
  });
